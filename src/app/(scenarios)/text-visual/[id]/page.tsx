...
export default function DynamicVisualizationPage({
  params,
}: {
  params: { id: string };
}) {
  ...
  // Listen for completion events from TaskPanel - now placed after goToNextStage is defined
  useEffect(() => {
      ...
      <div className="max-w-4xl mx-auto bg-white rounded-lg shadow-md p-8">
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-8">
          ...
          <div className="text-sm space-y-1">
            ...
            <div>
              <span className="font-semibold">Default Data Sources:</span>
              <div className="pl-4 text-xs mt-1 space-y-1">
                <div>Events: {metadata.dataSources.eventsDataPath}</div>
                <div>Quiz: {metadata.dataSources.quizDataPath}</div>
                <div>
                  Training Events: {metadata.dataSources.trainingEventsDataPath}
                </div>
                <div>
                  Training Quiz: {metadata.dataSources.trainingQuizDataPath}
                </div>
              </div>
            </div>
          </div>
